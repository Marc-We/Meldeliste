...
export function renderGroups() {
  if (!els.groupPreview) return;
  const groups = Array.isArray(state.groupPreview) ? state.groupPreview : null;
  if (!groups) {
    els.groupPreview.innerHTML = '';
    if (els.groupPublishBtn) els.groupPublishBtn.disabled = true;
    return;
  }
  if (!groups.length) {
    els.groupPreview.innerHTML = '<div class="small">Keine anwesenden Schüler im Raum.</div>';
    if (els.groupPublishBtn) els.groupPublishBtn.disabled = true;
    return;
  }
  // Reihenfolge: echte Gruppen (number>0) zuerst, Pool (number 0) zuletzt
  const ordered = [...groups].sort((a, b) => {
    const an = a.number === 0 ? Infinity : a.number;
    const bn = b.number === 0 ? Infinity : b.number;
    return an - bn;
  });
  els.groupPreview.innerHTML = ordered.map((g) => {
    ...
    const rows = (g.members || []).map((m) => {
      ...
      return `<div class="row" style="justify-content:space-between;align-items:center;gap:6px;margin-top:2px;">
        ...
        <span style="white-space:nowrap;">
          <button class="ghost" data-move-prev="${m.userId}" style="padding:2px 8px;">◀</button>
          <button class="ghost" data-move-next="${m.userId}" style="padding:2px 8px;">▶</button>
        </span>
      </div>`;
  ...
  // Verschiebe-Handler: Schüler in vorige/nächste Gruppe (Pool zählt als eine Station)
  const moveStudent = (userId, dir) => {
    const gs = state.groupPreview;
    if (!Array.isArray(gs)) return;
    // Zielreihenfolge der Gruppennummern (echte aufsteigend, Pool 0 als letzte Station)
  ...
  els.groupPreview.querySelectorAll('[data-move-prev]').forEach((btn) => {
    btn.onclick = () => moveStudent(btn.getAttribute('data-move-prev'), -1);
  });
  els.groupPreview.querySelectorAll('[data-move-next]').forEach((btn) => {
    btn.onclick = () => moveStudent(btn.getAttribute('data-move-next'), 1);
  });
